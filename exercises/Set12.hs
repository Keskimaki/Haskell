...
count n f = foldr (add n) 0 f
  where add n a acc = if n == a then acc + 1 else acc
...
instance Foldable TwoList where
  foldr f n TwoEmpty = n
  foldr f n (TwoNode x y xs) = f x (f y (foldr f n xs))
