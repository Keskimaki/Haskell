...
-- treeSize Empty = 0
-- treeSize (Node _ l r) = 1 + max (treeSize l) (treeSize r)
...
treeMax Empty = 0
treeMax (Node y l r) = maximum [y, treeMax l, treeMax r]
...
walk xs Empty = Nothing
walk [] (Node y _ _) = Just y
...
set xs val Empty = Empty
set [] val (Node y l r) = Node val l r
set (StepL:xs) val (Node y l r) = Node y (set xs val l) r
set (StepR:xs) val (Node y l r) = Node y l (set xs val r)
